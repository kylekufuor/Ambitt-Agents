...
/* ---------------------------------------------------------------------------
   Root layout for everything except the editorial pages (`/`, `/use-cases`),
   which have their own in app/(editorial)/layout.tsx so the two stylesheets
   never meet. Crossing between them is a full page load.

   Satoshi — ONE family, display through legal. No display/body pairing.
   The editorial layout loads this same file, as does the client portal, so a
   visitor moving from `/` to /docs or the legal pages never sees the
   letterforms change.
