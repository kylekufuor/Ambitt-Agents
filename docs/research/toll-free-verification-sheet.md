...
**APPROVED 5 Aug 2026.** `+18338536941` is verified and cleared to send.
Verification `HH5972b8b30e4e581dfb120515d25d8793`.

Filed 31 Jul → rejected 3 Aug (code 1407, "Opt-In Checkbox is Pre-selected" — our checkbox
never was, the *screenshot* was, having been taken mid-test with the box ticked) → evidence
regenerated and resubmitted 4 Aug → approved 5 Aug.

## Turn it on, in this order

The order is not cosmetic. Switching `TWILIO_SMS_NUMBER` first points the relay at a number
whose replies have nowhere to land, which is worse than the email fallback it replaces.

```bash
npx tsx scripts/tollfree-verify.ts --wire-webhook
```

Points the number's inbound webhook at `/webhooks/sms`. Without it, Casey's reply carrying the
code reaches Twilio and stops there: the relay hears nothing, the login expires, and nothing
surfaces the reason.

Then, and only then, set `TWILIO_SMS_NUMBER=+18338536941` on the Railway Oracle service.

Then Casey adds his mobile on the portal's Email setup page and ticks consent — still unset as
of 9 Aug, so the relay currently has nobody to text.

**What stayed true and is worth keeping:** the evidence image is generated by
`scripts/capture-optin-screenshot.ts`, which **refuses to write if the box is checked or the
number field is filled**. `EditReason` is NOT free text whatever the docs say — 193 chars
returns `400 Invalid edit reason`; Twilio's own example is `"Website fixed"`. And the approval
email only reached a human because of the unrouted-mail forwarder; `support@` would have
dropped it, as it dropped four days of the first edit window.
