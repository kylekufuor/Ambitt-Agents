...
//
// State is in-memory and NOT durable by design — an Oracle restart mid-MFA
// means the worker re-requests (matches the pre-existing design in
// oracle/index.ts). If Oracle ever goes multi-replica (Kubernetes), these maps
// move to Redis/DB behind the same function signatures.
//
...
// Hard cap: relay SMS per client per hour. On cap: skip SMS (fall to email)
// and alert the operator once per window. This in-memory cap is the SMS
// seatbelt until an SmsSend audit table lands (spec P1).
const SMS_HOURLY_CAP = 6;
...
// clientId → real SMS send timestamps inside the rolling hour (cap counter).
...
/** Rolling-hour SMS cap. `alertOperator` is true at most once per window. */
export function smsCapExceeded(clientId: string, now = Date.now()): { exceeded: boolean; alertOperator: boolean } {
  ...
  const alertOperator = !alerted || now - alerted >= HOUR_MS;
  if (alertOperator) capAlertedAt.set(clientId, now);
  return { exceeded: true, alertOperator };
}
 ...
 */
export async function relayMfaRequest(input: RelayMfaInput): Promise<RelayMfaResult> {
  ...
  for (const ch of order) {
    ...
    if (ch === "sms" && clientMobile) {
      const { smsConfigured, sendSms } = await import("./sms.js");
      if (!smsConfigured()) {
        logger.info("MFA relay: SMS not configured — trying next channel", { clientId });
        continue;
      }
      const cap = smsCapExceeded(clientId, now);
      if (cap.exceeded) {
        logger.warn("MFA relay: hourly SMS cap hit — trying next channel", { clientId, agentId });
        if (cap.alertOperator) {
          try {
            const { sendKyleWhatsApp } = await import("./whatsapp.js");
            await sendKyleWhatsApp(
              `⚠️ 2FA relay SMS cap hit for ${agent.client?.businessName || clientId} (${SMS_HOURLY_CAP}/hr). Agent: ${agent.name}. Falling back to email — check for a request loop.`
            );
          } catch (alertErr) {
            logger.warn("MFA relay: cap alert failed", { error: alertErr instanceof Error ? alertErr.message : String(alertErr) });
          }
        }
        continue;
      }
      ...
      try {
        if (agent.dryRun) {
          ...
          await sendSms({ to: clientMobile, message });
          recordSmsSend(clientId, now);
        }
      ...
      try {
        ...
        <p style="color:#9aa8a4;font-size:13px;">The code is used once and never stored. ${agent.name}, your agent at Ambitt</p>
          ...
          // Runtime mode: the paused run resumes when the client's email reply
          // flows through the normal inbound path — no pending2fa entry here.
          // Seatbelt-gated + dry-run-intercepted downstream in the router.
  ...
  if (channel === "none") {
    ...
    // instead of letting the task die quietly. Reaches Kyle by email today via
    // the sendKyleWhatsApp fallback.
    const tried = [clientMobile ? "sms" : null, clientEmail ? "email" : null].filter(Boolean).join(" + ") || "no channels on file";
    try {
      const { sendKyleWhatsApp } = await import("./whatsapp.js");
      await sendKyleWhatsApp(
        `🚨 2FA relay FAILED for ${agent.client?.businessName || clientId} — could not reach the client on any channel.\nService: ${service}\nAgent: ${agent.name} (${mode} path)\nTried: ${tried}`
      );
    } catch (alertErr) {
