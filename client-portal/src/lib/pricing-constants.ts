// ---------------------------------------------------------------------------
// Ambitt Agents — Pricing Constants (portal mirror)
// ---------------------------------------------------------------------------
// MANUAL MIRROR OF `shared/pricing-constants.ts`. The portal is deployed on
// Railway with `rootDirectory: client-portal/`, so it cannot reach files
// outside that root at build time. Until we set up a proper monorepo package
// (npm workspaces / Turborepo), any pricing change must be made in BOTH:
//   - `shared/pricing-constants.ts`              (canonical — Oracle + dashboard)
//   - `client-portal/src/lib/pricing-constants.ts` (this file — portal)
