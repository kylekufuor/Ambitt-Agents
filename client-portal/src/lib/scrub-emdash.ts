// ---------------------------------------------------------------------------
// Ambitt Agents — em dash scrub (portal mirror)
// ---------------------------------------------------------------------------
// MANUAL MIRROR OF `shared/scrub-emdash.ts`. The portal is deployed on
// Railway with `rootDirectory: client-portal/`, so it cannot reach files
// outside that root at build time. Any change must be made in BOTH:
//   - `shared/scrub-emdash.ts`                 (canonical — Oracle + runtime)
//   - `client-portal/src/lib/scrub-emdash.ts`  (this file — portal)
// File contents below must stay byte-identical to the canonical version.
//
// Why the portal needs it at all: the ban was enforced only at email send
// time, so the same agent-authored sentence rendered clean in a letter and
// with a dash on a lead card. All three of Casey's live leads are named
// "<Entity> — <Property>".
// ---------------------------------------------------------------------------
