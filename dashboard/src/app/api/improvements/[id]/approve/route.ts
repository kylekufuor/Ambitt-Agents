...
// Thin proxy to Oracle's POST /improvements/:id/approve.
...
export async function POST(
  ...
  const upstream = await fetch(`${oracleUrl()}/improvements/${id}/approve`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: upstreamBody,
    cache: "no-store",
  }).catch((err) => {
    console.error("[dashboard/improvements approve] Oracle POST failed", err);
    return null;
  });
  if (!upstream) {
